...
void main()
{
    int nStrRef = StringToInt(GetScriptParam(M_LSE_SIN_PARAM_STRREF));
    ...
    string sString = nStrRef == 1   ? "Ich sollte mich nach Schätzen umsehen."
                   ...
                   : nStrRef == 601  ? "Erkläre mir, wie ich M_LSE verwende!"
                   : nStrRef == 602  ? "Alle Features von M_LSE wurden automatisch installiert. Es muss nichts weiter unternommen werden, es sei denn du möchtest bestimmte Features umschalten oder einstellen, was im Folgenden erläutert wird."
                   : nStrRef == 603  ? "Um mit M_LSE zu interagieren, finde und benutze das \"Spieler Werkzeug 9\" unter den \"Spezialfähigkeiten\" in deinem Rechtsklick-Menü. Du wirst dann ein Ziel auswählen müssen, wovon die Art der Interaktion abhängt. Treffe jedoch zunächst keine Auswahl, das würde nämlich dieses Tutorial schließen!"
                   : nStrRef == 604  ? "Was passiert, wenn ich meinen Charakter auswähle?"
                   : nStrRef == 605  ? "Was passiert, wenn ich eine Leiche auswähle?"
                   : nStrRef == 606  ? "Ich weiß nun, wie ich M_LSE verwende. Schließe das Tutorial."
