...
const string MELSE_LOCAL_FEATURE_LOOT_ALL_ARROWS = "MELSE_FEATURE_LOOT_ALL_ARROWS";
const string MELSE_LOCAL_FEATURE_LOOT_ALL_BOLTS = "MELSE_FEATURE_LOOT_ALL_BOLTS";
const string MELSE_LOCAL_FEATURE_LOOT_ALL_BULLETS = "MELSE_FEATURE_LOOT_ALL_BULLETS";
const string MELSE_LOCAL_FEATURE_LOOT_ALL_BOOKS = "MELSE_FEATURE_LOOT_ALL_BOOKS";
...
const string MELSE_LOCAL_CORPSE_LOOT_DISPLAY = "MELSE_CORPSE_LOOT_DISPLAY";
...
const string MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME = "MELSE_CORPSE_INVENTORY_OWNER_NAME";
...
const string MELSE_LOCAL_DEBUG_INITIALIZE_COUNT = "MELSE_DEBUG_INITIALIZE_COUNT";
const string MELSE_LOCAL_DEBUG_DISPLAY_UPDATE_COUNT = "MELSE_DEBUG_DISPLAY_UPDATE_COUNT";
const string MELSE_LOCAL_DEBUG_DISPLAY_CANDIDATE_COUNT = "MELSE_DEBUG_DISPLAY_CANDIDATE_COUNT";
const string MELSE_LOCAL_DEBUG_DISPLAY_RESULT = "MELSE_DEBUG_DISPLAY_RESULT";
...
const int MELSE_CORPSE_HILITE_COLOR = 0x00FFFF;
const int MELSE_CORPSE_DISPLAY_ITEM_LIMIT = 8;
const int MELSE_CORPSE_DISPLAY_RESULT_FEATURE_DISABLED = 1;
const int MELSE_CORPSE_DISPLAY_RESULT_NOT_DEAD = 2;
const int MELSE_CORPSE_DISPLAY_RESULT_NOT_LOOTABLE = 3;
const int MELSE_CORPSE_DISPLAY_RESULT_NO_CANDIDATES = 4;
const int MELSE_CORPSE_DISPLAY_RESULT_DISPLAYED = 5;
const int MELSE_CORPSE_DISPLAY_RESULT_INVENTORY_UNAVAILABLE = 6;
...
const string MELSE_INJECT_KEY_MODULE_ACQUIRED_ITEM = "melse.module.acquire";
const string MELSE_INJECT_KEY_AREA_ENTER = "melse.area.enter";
const string MELSE_INJECT_KEY_CREATURE_DEATH = "melse.creature.death";
const string MELSE_INJECT_KEY_PLACEABLE_CLOSE = "melse.placeable.close";
const string MELSE_INJECT_KEY_PLACEABLE_OPEN = "melse.placeable.open";
const string MELSE_INJECT_KEY_PLACEABLE_UNLOCK = "melse.placeable.unlock";
const string MELSE_RUNTIME_MARKER_MODULE_EVENTS = "melse.registration.module";
const string MELSE_RUNTIME_MARKER_AREA_OBJECTS = "melse.registration.area.objects";
...
string MELSE_GetLootSourceName(object oObject)
{
    string sName = GetName(oObject);
    string sOwnerName = GetLocalString(oObject, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME);
    return sOwnerName == "" ? sName : sName + ": " + sOwnerName;
}
...
string MELSE_GetText(int iKey, object oObject = OBJECT_INVALID, int iStackSize = 0, object oItem = OBJECT_INVALID)
{
    ...
    if (iKey == MELSE_TEXT_ITEM_TAKEN)
        sResult = StringToRGBString(MELSE_GetLootSourceName(oObject) + " >> " +
...
{
    if (!GetIsObjectValid(oCorpse) || GetObjectType(oCorpse) != OBJECT_TYPE_CREATURE || !GetIsDead(oCorpse))
        return OBJECT_INVALID;

    object oInventory = GetLocalObject(oCorpse, MELSE_LOCAL_CORPSE_INVENTORY);
    if (GetIsObjectValid(oInventory))
    {
        if (GetLocalString(oInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME) == "")
            SetLocalString(oInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME, GetName(oCorpse));
        return oInventory;
    }

    // The engine exposes a lootable dead creature's loot through its hidden BodyBagId inventory.
    json jCreature = ObjectToJson(oCorpse);
    json jBodyBagId = JsonObjectGet(jCreature, "BodyBagId");
    int nBodyBagId = JsonGetInt(JsonObjectGet(jBodyBagId, "value"));
    oInventory = StringToObject(IntToHexString(nBodyBagId));
    if (!GetIsObjectValid(oInventory) || !GetHasInventory(oInventory))
        return OBJECT_INVALID;

    SetLocalObject(oCorpse, MELSE_LOCAL_CORPSE_INVENTORY, oInventory);
    SetLocalObject(oInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER, oCorpse);
    SetLocalString(oInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME, GetName(oCorpse));
    return oInventory;
}
...
int MELSE_GetHasAnyItem(object oObject)
{
    ...
    if (GetObjectType(oObject) == OBJECT_TYPE_CREATURE && GetIsDead(oObject) && GetLootable(oObject))
    {
        object oInventory = MELSE_GetCorpseInventory(oObject);
        // Fail closed: an unresolved corpse inventory must never make a corpse eligible for removal.
        if (!GetIsObjectValid(oInventory))
            return TRUE;
        return GetIsObjectValid(GetFirstItemInInventory(oInventory)) || GetGold(oObject) > 0 || GetGold(oInventory) > 0;
    }
...
string MELSE_BuildInventoryLootText(object oInventory, object oDisplayObject)
{
    ...
    {
        if (iItems < MELSE_CORPSE_DISPLAY_ITEM_LIMIT)
            sText += "\n- " + MELSE_GetCorpseItemLabel(oItem);
        else
            bMore = TRUE;
        iItems++;
...
void MELSE_RemoveLegacyCorpseGlow(object oCorpse)
{
    effect eEffect = GetFirstEffect(oCorpse);
    while (GetIsEffectValid(eEffect))
    {
        effect eNext = GetNextEffect(oCorpse);
        if (GetEffectTag(eEffect) == "MELSE_CORPSE_GLOW")
            RemoveEffect(oCorpse, eEffect);
        eEffect = eNext;
    }
}
...
void MELSE_ApplyLootDisplay(object oDisplayObject, string sText, int iTextMode = OBJECT_UI_TEXT_BUBBLE_OVERRIDE_APPEND)
{
    if (sText == "")
    {
    ...
    SetObjectTextBubbleOverride(oDisplayObject, iTextMode, sText);
}
...
void MELSE_UpdateInventoryLootDisplay(object oInventory)
{
    if (!GetIsObjectValid(oInventory) || !GetHasInventory(oInventory))
        return;
    string sText = MELSE_BuildInventoryLootText(oInventory, oInventory);
    string sOwnerName = GetLocalString(oInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME);
    if (GetTag(oInventory) == MELSE_TAG_BODYBAG && sOwnerName != "" && sText != "")
        MELSE_ApplyLootDisplay(oInventory, GetName(oInventory) + ": " + sOwnerName + sText, OBJECT_UI_TEXT_BUBBLE_OVERRIDE_REPLACE);
    else
        MELSE_ApplyLootDisplay(oInventory, sText);
}
...
{
    if (!GetIsObjectValid(oCorpse) || GetObjectType(oCorpse) != OBJECT_TYPE_CREATURE)
        return;
    ...
    else if (!RAV_GetLocalInt(oCorpse, MELSE_LOCAL_MOD_LOOTABLE))
    {
        iResult = MELSE_CORPSE_DISPLAY_RESULT_NOT_LOOTABLE;
    }
    else if (!GetIsObjectValid(MELSE_GetCorpseInventory(oCorpse)))
    {
        iResult = MELSE_CORPSE_DISPLAY_RESULT_INVENTORY_UNAVAILABLE;
    }
    else
    {
...
int MELSE_GetIsItemLootingObligatory(object oItem)
{
    ...
    bResult = nBaseItemType == BASE_ITEM_CRAFTMATERIALMED   ? TRUE
            ...
            : nBaseItemType == BASE_ITEM_ARROW              ? MELSE_GetConfigInt(MELSE_LOCAL_FEATURE_LOOT_ALL_ARROWS)
            : nBaseItemType == BASE_ITEM_BOLT               ? MELSE_GetConfigInt(MELSE_LOCAL_FEATURE_LOOT_ALL_BOLTS)
            : nBaseItemType == BASE_ITEM_BULLET             ? MELSE_GetConfigInt(MELSE_LOCAL_FEATURE_LOOT_ALL_BULLETS)
...
void MELSE_GiveItem(object oItem, object oTakeFrom, object oGiveTo)
{
...
int MELSE_TakeItem(object oItem, object oTakeFrom, object oGiveTo, int bIfNotExcludedByConfig = TRUE)
{
    if (!bIfNotExcludedByConfig || !MELSE_GetIsLootingExcluded(oItem))
    {
        ...
        AssignCommand(oTakeFrom, MELSE_GiveItem(oItem, oTakeFrom, oGiveTo));
...
int MELSE_TakeDroppables(object oTakeFrom, object oGiveTo)
{
    ...
    if (!MELSE_GetIsLootingExcluded(oTakeFrom))
    {
        ...
        AssignCommand(oTakeFrom, ActionDoCommand(MELSE_ConfirmObjectLooted(oTakeFrom)));
    }
    return bResult;
}
...
void MELSE_MAINTENANCE_SetDefaultConfig(object oModule)
{
    ...
    MELSE_SetConfigInt(MELSE_LOCAL_FEATURE_LOOT_ALL_ARROWS, FALSE);
...
void MELSE_MAINTENANCE_Build(object oModule, object oPC, string sNewBuild, string sOldBuild = STRING_EMPTY)
{
    ...
    if (sOldBuild == "1013")
    {
        // Build 1013 briefly used a persistent model glow. Remove it once from existing saves.
        object oArea = GetFirstArea();
        while (GetIsObjectValid(oArea))
        {
            object oObject = GetFirstObjectInArea(oArea);
            ...
            {
                if (GetObjectType(oObject) == OBJECT_TYPE_CREATURE && GetIsDead(oObject))
                    MELSE_RemoveLegacyCorpseGlow(oObject);
                oObject = GetNextObjectInArea(oArea);
            }
            oArea = GetNextArea();
        }
...
{
    if (ESI_IsRuntimeMarkerSet(oModule, MELSE_RUNTIME_MARKER_MODULE_EVENTS)) return;
    int bRegistered = TRUE;
    if (!ESI_InjectToObject(oModule, MELSE_INJECT_KEY_MODULE_ACQUIRED_ITEM, EVENT_SCRIPT_MODULE_ON_ACQUIRE_ITEM, MELSE_SCRIPT_EVENT_MODULE_ACQUIRED_ITEM, ESI_INJECTION_PLACEMENT_LAST)) bRegistered = FALSE;
    object oArea = GetFirstArea();
    while (GetIsObjectValid(oArea))
    {
        if (!ESI_InjectToObject(oArea, MELSE_INJECT_KEY_AREA_ENTER, EVENT_SCRIPT_AREA_ON_ENTER, MELSE_SCRIPT_EVENT_AREA_ENTER, ESI_INJECTION_PLACEMENT_LAST)) bRegistered = FALSE;
        oArea = GetNextArea();
    }
    if (bRegistered) ESI_SetRuntimeMarker(oModule, MELSE_RUNTIME_MARKER_MODULE_EVENTS);
}

void MELSE_InjectAreaObjects(object oArea)
{
    if (!GetIsObjectValid(oArea) || ESI_IsRuntimeMarkerSet(oArea, MELSE_RUNTIME_MARKER_AREA_OBJECTS)) return;
    int bRegistered = TRUE;
    object oObject = GetFirstObjectInArea(oArea);
    while (GetIsObjectValid(oObject))
    {
        ...
        {
            if (!ESI_InjectToObject(oObject, MELSE_INJECT_KEY_CREATURE_DEATH, EVENT_SCRIPT_CREATURE_ON_DEATH, MELSE_SCRIPT_EVENT_CREATURE_DEATH, ESI_INJECTION_PLACEMENT_LAST)) bRegistered = FALSE;
        }
        else if (nObjectType == OBJECT_TYPE_PLACEABLE)
        {
            if (!ESI_InjectToObject(oObject, MELSE_INJECT_KEY_PLACEABLE_OPEN, EVENT_SCRIPT_PLACEABLE_ON_OPEN, MELSE_SCRIPT_EVENT_PLACEABLE_OPEN, ESI_INJECTION_PLACEMENT_LAST)) bRegistered = FALSE;
            if (!ESI_InjectToObject(oObject, MELSE_INJECT_KEY_PLACEABLE_CLOSE, EVENT_SCRIPT_PLACEABLE_ON_CLOSED, MELSE_SCRIPT_EVENT_PLACEABLE_CLOSE, ESI_INJECTION_PLACEMENT_FIRST)) bRegistered = FALSE;
            if (!ESI_InjectToObject(oObject, MELSE_INJECT_KEY_PLACEABLE_UNLOCK, EVENT_SCRIPT_PLACEABLE_ON_UNLOCK, MELSE_SCRIPT_EVENT_PLACEABLE_UNLOCK, ESI_INJECTION_PLACEMENT_LAST)) bRegistered = FALSE;
        }
        oObject = GetNextObjectInArea(oArea);
    }
    if (bRegistered) ESI_SetRuntimeMarker(oArea, MELSE_RUNTIME_MARKER_AREA_OBJECTS);
}
...
void MELSE_Initialize(object oModule, object oPC)
{
    ...
    MELSE_InjectAreaObjects(GetArea(oPC));
}
...
void MELSE_FinalizeCorpseInitialization(object oCorpse)
{
...
void MELSE_ReconcileCorpseLoot(object oCorpse, object oBodyBag, object oRecipient)
{
    MELSE_TakeDroppables(oBodyBag, oRecipient);
    ActionDoCommand(MELSE_FinalizeCorpseInitialization(oCorpse));
}

void MELSE_InitializeCorpse(object oCorpse, object oRecipient = OBJECT_INVALID)
{
    ...
    if (GetIsObjectValid(oBodyBag) && GetIsObjectValid(oRecipient))
    {
        AssignCommand(oBodyBag, MELSE_ReconcileCorpseLoot(oCorpse, oBodyBag, oRecipient));
        return;
    }
    MELSE_FinalizeCorpseInitialization(oCorpse);
}
...
{
    object oOldInventory = GetLocalObject(oObject, MELSE_LOCAL_CORPSE_INVENTORY);
    if (GetIsObjectValid(oOldInventory))
    {
        DeleteLocalObject(oOldInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER);
        DeleteLocalString(oOldInventory, MELSE_LOCAL_CORPSE_INVENTORY_OWNER_NAME);
    }
    ...
    else if (MELSE_GetConfigInt(MELSE_LOCAL_FEATURE_CORPSE_LOOTING_KILLED_BY_HENCH))
    {
        ...
        if (GetIsPC(oMaster))
            ...
            oRecipient = oSuperMaster;
    }
    if (GetIsObjectValid(oRecipient))
        bLootedAll = MELSE_TakeDroppables(oObject, oRecipient);
    ...
    DelayCommand(MELSE_DELAY_CORPSE_INITIALIZE, MELSE_InitializeCorpse(oObject, GetLootable(oObject) ? oRecipient : OBJECT_INVALID));
}
