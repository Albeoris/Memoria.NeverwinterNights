...
const int M_TACT_ROLE_GENERIC = 0;
const int M_TACT_ROLE_SUMMON = 1;
const int M_TACT_ROLE_NEGATIVE = 2;

int M_TACT_GetSpellRole(int iSpell)
{
...
int M_TACT_GetActionSpellLevel(json jAction, int iSpell)
{
...
float M_TACT_GetSpellAreaRadius(object oCaster, int iSpell, int iSpellLevel, int iMetaMagic)
{
    float fRadius = StringToFloat(Get2DAString("spells", "TargetSizeX", iSpell));
    return fRadius > 0.0f ? fRadius : 3.33f;
}

float M_TACT_GetSpellRange(int iSpell)
{
...
int M_TACT_ImmunityNameToType(string sImmunity)
{
...
int M_TACT_ImmunityNameToDamageType(string sImmunity, int iSpell)
{
...
int M_TACT_DamageTypeToItemType(int iDamageType)
{
...
int M_TACT_HasFullDamageImmunity(object oTarget, int iDamageType)
{
    ...
    int iItemType = M_TACT_DamageTypeToItemType(iDamageType);
    int iSlot;
...
int M_TACT_IsSpellSensible(object oCaster, object oTarget, int iSpell, int iSpellLevel)
{
    DeleteLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON);
    if (!GetIsObjectValid(oCaster) || !GetIsObjectValid(oTarget))
    {
        SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "invalid_object");
        return FALSE;
    }
    if (M_TACT_IsSpellHostile(iSpell))
    {
        if (M_TACT_GetSpellRole(iSpell) == M_TACT_ROLE_NEGATIVE && GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD)
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "negative_vs_undead");
            return FALSE;
        }
        string sImmunity = Get2DAString("spells", "ImmunityType", iSpell);
        int iImmunity = M_TACT_ImmunityNameToType(sImmunity);
        if (iImmunity != IMMUNITY_TYPE_NONE && GetIsImmune(oTarget, iImmunity, oCaster))
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "status_immunity");
            return FALSE;
        }
        if (M_TACT_HasFullDamageImmunity(oTarget, M_TACT_ImmunityNameToDamageType(sImmunity, iSpell)))
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "damage_immunity");
            return FALSE;
        }
        if (SpellImmunityCheck(oTarget, oCaster, iSpell, FALSE))
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "spell_immunity");
            return FALSE;
        }
        if (SpellAbsorptionLimitedCheck(oTarget, oCaster, iSpell, -1, iSpellLevel, FALSE, FALSE))
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "limited_absorption");
            return FALSE;
        }
        if (SpellAbsorptionUnlimitedCheck(oTarget, oCaster, iSpell, -1, iSpellLevel, FALSE))
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "unlimited_absorption");
            return FALSE;
        }
        int iResistance = GetSpellResistance(oTarget);
        if (iResistance > 0 && iResistance > GetCasterLevel(oCaster) + 20)
        {
            SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "unbeatable_spell_resistance");
            return FALSE;
        }
    }
    else if (M_TACT_GetSpellRole(iSpell) != M_TACT_ROLE_SUMMON && GetHasSpellEffect(iSpell, oTarget))
    {
        SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "effect_already_present");
        return FALSE;
    }
    SetLocalInt(oCaster, "M_TACT_META_SPELL", iSpell);
    SetLocalObject(oCaster, "M_TACT_META_TARGET", oTarget);
    ExecuteScript("m_tact_meta_ext", oCaster);
    DeleteLocalInt(oCaster, "M_TACT_META_SPELL");
    DeleteLocalObject(oCaster, "M_TACT_META_TARGET");
    int iDecision = GetLocalInt(oCaster, "M_TACT_META_DECISION");
    DeleteLocalInt(oCaster, "M_TACT_META_DECISION");
    if (iDecision < 0) SetLocalString(oCaster, M_TACT_LOCAL_REJECT_REASON, "extension_rejected");
