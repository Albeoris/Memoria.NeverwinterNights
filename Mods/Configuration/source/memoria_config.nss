...
{
    object oModule = GetModule();
    if (!GetLocalInt(oModule, MEMORIA_CONFIG_LOCAL_ESI_INSTALLED) && ESI_InjectToObject(oModule, "config", EVENT_SCRIPT_MODULE_ON_ACTIVATE_ITEM, MEMORIA_CONFIG_ACTIVATE_HANDLER, ESI_INJECTION_PLACEMENT_FIRST))
        SetLocalInt(oModule, MEMORIA_CONFIG_LOCAL_ESI_INSTALLED, TRUE);
}
