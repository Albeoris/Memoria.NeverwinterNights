...
const string MEBOOTSTRAPPER_MANIFEST_PREFIX = "memoria_";
const string MEBOOTSTRAPPER_DISCOVERY_LOCAL = "MEBOOTSTRAPPER_DISCOVERY";
const string MEBOOTSTRAPPER_CACHE_OWNER_LOCAL = "MEBOOTSTRAPPER_CACHE_OWNER";
const string MEBOOTSTRAPPER_CACHE_WINDOW = "meboot_cache";
const string MEBOOTSTRAPPER_DISPATCH_CYCLE_LOCAL = "MEBOOTSTRAPPER_DISPATCH_CYCLE";
const string MEBOOTSTRAPPER_DISPATCH_DONE_LOCAL = "MEBOOTSTRAPPER_DISPATCH_DONE_";
const string MEBOOTSTRAPPER_DISPATCH_FAILURE_LOCAL = "MEBOOTSTRAPPER_DISPATCH_FAILURE_";

void MEBOOTSTRAPPER_ReportError(object oPlayer, string sManifest, string sError)
{
    string sLocal = "MEBOOTSTRAPPER_ERR_" + sManifest;
...
int MEBOOTSTRAPPER_HasId(json jEntries, string sId)
{
    int nIndex;
...
json MEBOOTSTRAPPER_DiscoverEntries(object oPlayer)
{
    json jEntries = JsonArray();
    int nNth = 1;
    string sManifest = ResManFindPrefix(MEBOOTSTRAPPER_MANIFEST_PREFIX, RESTYPE_TXT, nNth, FALSE);
    while (sManifest != "")
    {
        ...
        nNth++;
        sManifest = ResManFindPrefix(MEBOOTSTRAPPER_MANIFEST_PREFIX, RESTYPE_TXT, nNth, FALSE);
    }
    return jEntries;
}

json MEBOOTSTRAPPER_BuildCacheWindow()
{
...
json MEBOOTSTRAPPER_GetEntries(object oPlayer)
{
    // NUI user data is server-side and expires with its window, so the cache cannot be restored from a save game.
    object oModule = GetModule();
    object oCacheOwner = GetLocalObject(oModule, MEBOOTSTRAPPER_CACHE_OWNER_LOCAL);
    int nToken = GetIsObjectValid(oCacheOwner) ? NuiFindWindow(oCacheOwner, MEBOOTSTRAPPER_CACHE_WINDOW) : 0;
    json jEntries = NuiGetUserData(oCacheOwner, nToken);
    if (nToken > 0 && JsonGetType(jEntries) == JSON_TYPE_ARRAY) return jEntries;

    jEntries = MEBOOTSTRAPPER_DiscoverEntries(oPlayer);
    oCacheOwner = oPlayer;
    nToken = NuiFindWindow(oCacheOwner, MEBOOTSTRAPPER_CACHE_WINDOW);
    if (nToken == 0) nToken = NuiCreate(oCacheOwner, MEBOOTSTRAPPER_BuildCacheWindow(), MEBOOTSTRAPPER_CACHE_WINDOW, "meboot_noop");
    if (nToken > 0)
    {
        NuiSetUserData(oCacheOwner, nToken, jEntries);
        SetLocalObject(oModule, MEBOOTSTRAPPER_CACHE_OWNER_LOCAL, oCacheOwner);
    }
    return jEntries;
}

void MEBOOTSTRAPPER_CheckDispatch(object oPlayer, string sId, string sHeartbeat, string sDoneLocal, string sFailureLocal)
{
...
void MEBOOTSTRAPPER_Dispatch(object oPlayer, string sId, string sHeartbeat, int nCycle, int nIndex)
{
    string sDoneLocal = MEBOOTSTRAPPER_DISPATCH_DONE_LOCAL + IntToString(nCycle) + "_" + IntToString(nIndex);
    string sFailureLocal = MEBOOTSTRAPPER_DISPATCH_FAILURE_LOCAL + IntToString(nIndex);
    DeleteLocalInt(oPlayer, sDoneLocal);
    DelayCommand(0.1f, MEBOOTSTRAPPER_CheckDispatch(oPlayer, sId, sHeartbeat, sDoneLocal, sFailureLocal));
...
void main()
{
    ...
    json jEntries = MEBOOTSTRAPPER_GetEntries(oPlayer);
    int nCycle = GetLocalInt(oPlayer, MEBOOTSTRAPPER_DISPATCH_CYCLE_LOCAL) + 1;
    SetLocalInt(oPlayer, MEBOOTSTRAPPER_DISPATCH_CYCLE_LOCAL, nCycle);
    ...
    for (nIndex = 0; nIndex < JsonGetLength(jEntries); nIndex++)
    {
        ...
        DelayCommand(IntToFloat(nIndex) * 0.01f, MEBOOTSTRAPPER_Dispatch(oPlayer, sId, sHeartbeat, nCycle, nIndex));
        sDetected = sDetected == "" ? sId : sDetected + "," + sId;
    }
    if (GetLocalString(oPlayer, MEBOOTSTRAPPER_DISCOVERY_LOCAL) != sDetected)
    {
        SetLocalString(oPlayer, MEBOOTSTRAPPER_DISCOVERY_LOCAL, sDetected);
