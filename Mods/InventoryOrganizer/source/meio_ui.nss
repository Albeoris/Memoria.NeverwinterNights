...
json MEIO_BuildWindow(object oPC, json jCapacities)
{
    ...
    jRoot = JsonArrayInsert(jRoot, NuiHeight(NuiRow(jFilters), 30.0f));
    ...
    jRoot = JsonArrayInsert(jRoot, NuiHeight(NuiGroup(NuiCol(jRows), TRUE, NUI_SCROLLBARS_Y), 322.0f));
    json jFooter = JsonArray();
    jFooter = JsonArrayInsert(jFooter, NuiWidth(NuiId(NuiButton(JsonString(MEIO_GetText(oPC, "return_scrolls"))), "return_scrolls"), 190.0f));
    jFooter = JsonArrayInsert(jFooter, NuiSpacer());
    jFooter = JsonArrayInsert(jFooter, NuiWidth(NuiLabel(JsonString(MEIO_GetText(oPC, "withdraw_hint")), JsonInt(NUI_HALIGN_RIGHT), JsonInt(NUI_VALIGN_MIDDLE)), 430.0f));
    jRoot = JsonArrayInsert(jRoot, NuiHeight(NuiRow(jFooter), 32.0f));
    return NuiWindow(NuiCol(jRoot), JsonString(MEIO_GetText(oPC, "window_title")), NuiRect(-1.0f, -1.0f, 760.0f, 470.0f), JsonBool(FALSE), JsonBool(FALSE), JsonBool(TRUE), JsonBool(FALSE), JsonBool(TRUE));
}
...
void MEIO_Open(object oPC)
{
    ...
    int iToken = MEMORIA_NUI_Create(oPC, MEIO_BuildWindow(oPC, jCapacities), MEIO_WINDOW, "meio_nuievt");
    if (iToken > 0)
    {
